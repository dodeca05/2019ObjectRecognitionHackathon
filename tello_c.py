...
class Tello():
    ...
    def send_command(self, command):
        ...
        if self.socket is None:
            ...
            return False,self.sendcount
        else:
            try:
                self.socket.sendto(command.encode('utf-8'), self.tello_address)
                self.loggingFunc('[%d]:요청: %s to %s' % (self.sendcount,command, self.tello_address[0]))
                return True,self.sendcount
        ...
        while True:
            if self.socket is not None:
                try:
                    ...
                    self.loggingFunc('[',self.revcount,']:응답: %s' % self.response)
